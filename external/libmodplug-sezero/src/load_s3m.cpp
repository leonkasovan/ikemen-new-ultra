...
//---------------------------------------------------------------
{
	...
	for (UINT iSmp=1; iSmp<=insnum; iSmp++)
	{
		...
		{
			Ins[iSmp].nLength = boundInput(bswapLE32(pSmp.length), 4U, MAX_SAMPLE_LENGTH);
			Ins[iSmp].nLoopStart = boundInput(bswapLE32(pSmp.loopbegin), 4U, Ins[iSmp].nLength - 1);
			Ins[iSmp].nLoopEnd = boundInput(bswapLE32(pSmp.loopend), 4U, Ins[iSmp].nLength);
