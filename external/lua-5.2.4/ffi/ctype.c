...
void* push_cdata(lua_State* L, int ct_usr, const struct ctype* ct)
{
    ...
    *(struct ctype*) &cd->type = *ct;
    cd->magic = 0xDEADBEEF;  /* Sentinel to detect freed memory */
    cd->is_valid = 1;        /* Mark cdata as valid on creation */
    memset(cd+1, 0, sz);
...
void* to_cdata(lua_State* L, int idx, struct ctype* ct)
{
    ...
    /* Safety check: prevent access to freed SSZ memory (Option B)
     * Check magic sentinel (0xDEADBEEF) + is_valid flag to catch freed memory */
    if (!cd || cd->magic != 0xDEADBEEF || !cd->is_valid) {
        lua_pushnil(L);
        return NULL;
    }
    
    /* Use memcpy to avoid alignment-sensitive SSE (movdqa) instructions that GCC
     * may emit for direct struct assignment when the struct has over-alignment.
     * lua_newuserdata does not guarantee 16-byte alignment, so cd->type may be
     * only 8-byte aligned, causing SIGSEGV on movdqa. */
