...
static int cdata_gc(lua_State* L)
{
    ...
    /* Safety check: prevent access to freed SSZ memory before calling check_cdata */
    ...
    /* If sentinel is corrupted or is_valid is cleared, skip finalization */
    if (!cd || cd->magic != 0xDEADBEEF || !cd->is_valid) {
        return 0;  /* Skip finalizer for invalid/freed cdata */
    }
...
static int cdata_tostring(lua_State* L)
{
    ...
    case INT64_TYPE:
        if (ct.is_unsigned) {
            sprintf(buf, "%llu", (unsigned long long) *(uint64_t*) p);
        } else {
            sprintf(buf, "%lld", (long long) *(int64_t*) p);
        }
    ...
    default:
        if (ct.is_unsigned) {
            sprintf(buf, "%llu", (unsigned long long) check_intptr(L, 1, p, &ct));
        } else {
            sprintf(buf, "%lld", (long long) check_intptr(L, 1, p, &ct));
        }
        lua_pushstring(L, buf);
        return 1;
    }
...
static int ffi_cleanup(lua_State* L);
...
static const luaL_Reg ffi_reg[] = {
    ...
    {"i64", &ffi_i64},
    {"u64", &ffi_u64},
    {"cleanup", &ffi_cleanup},  /* Option C: invalidate all cdata before lua_close */
    {NULL, NULL}
};
...
/* ffi_cleanup: Mark all cdata objects as invalid and clear gc table to prevent finalizers (Option C) */
static int ffi_cleanup(lua_State* L)
{
    /* Get the gc table (all cdata objects are stored here) */
    push_upval(L, &gc_key);
    
    if (lua_istable(L, -1)) {
        lua_pushnil(L);
        while (lua_next(L, -2)) {
            /* Stack: gc_table, key, userdata */
            if (lua_isuserdata(L, -1)) {
                struct cdata* cd = (struct cdata*) lua_touserdata(L, -1);
                if (cd) {
                    cd->magic = 0;      /* Clear sentinel to mark invalid */
                    cd->is_valid = 0;   /* Invalidate cdata to prevent access to freed memory */
                }
            }
            lua_pop(L, 1);  /* Remove value, keep key for next iteration */
        }
        
        /* Clear the entire gc_key table to prevent lua_close from finding any cdata to finalize */
        lua_newtable(L);
        lua_replace(L, -2);  /* Replace gc table with empty table */
        set_upval(L, &gc_key);  /* Update the upval */
    } else {
        lua_pop(L, 1);
    }
