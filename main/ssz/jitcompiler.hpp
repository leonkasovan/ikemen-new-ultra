...
class JITer
{
	...
	public:
			...
			void* (SSZ_STDCALL* newer)())
		{
			...
			SSZ_TRY
				if(newer == nullptr && fp == RefClassNewCB){
					r->refnew(l, ts, refzeroclearcb);
				}else{
					r->refnew(l, ts, fp, &rn);
				}
...
LOOPENTER:
			...
			SrcTree*& retcall)
		{
			...
			case KAKKOCOLOOPEN_TOKEN:
				...
				tystack.pop_back();
				{
				intptr_t _dbg_fi = i; // capture funcId position before KansuuPointer advances i
				if(
					(
						tmpi = KansuuPointer(
							stree, iap, i, tystack,
...
public:
	...
	MEMBER MAIN_FUNC Compile(std::basic_string<LineAddress>* la)
	{
		...
		// The loop above skips the last node in the subsrc chain (its subsrc
		// is nullptr).  For a single-file script this is the root itself.
		// Set its golbalgeta and account for its global-data size so that
		// gre is large enough and GlobalAddress references are correct.
		pst->extroot->golbalgeta = ttl;
		ttl += pst->GetMemberSize();
