...
// Global memory-management function pointers.
// Defined once in ssz.cpp; every other TU sees these extern declarations
// so that COMDAT-folded inline functions from arrayandref.hpp always
// reference the same (initialised) pointers.
extern void* (SSZ_STDCALL *sszrefnewfunc)(intptr_t);
extern void (SSZ_STDCALL *sszrefdeletefunc)(void*);

// =========================================================================
// Global byte-level allocation counters  (defined in ssz.cpp)
// =========================================================================
...
// =========================================================================
//   LOG_INFO(tag, ...)    — always-on to stdout
//   LOG_DEBUG(tag, ...)   — debug-only (compiled to nothing in Release)
// =========================================================================
#define LOG_INFO(tag, fmt, ...)  do{ printf("[" tag "] " fmt "\n", ##__VA_ARGS__); fflush(stdout); }while(0)

#if defined(DEBUG) || defined(DEBUG)
#define LOG_DEBUG(tag, fmt, ...)   do{ printf("[DEBUG][" tag "] " fmt "\n", ##__VA_ARGS__); fflush(stdout); }while(0)
#else
