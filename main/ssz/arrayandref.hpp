...
//////////////////////////////////////////////////////////////////////////////
	...
	{
		if(tsize <= 0){
			LOG_DEBUG("SSZ", "Reference::listnewat invalid tsize=%d i=%d", (int)tsize, (int)i);
			return nullptr;
		}
		intptr_t idx;
		if(i < 0){
			idx = addsize(-i, tsize, fp, p);
			if(null() || obj() == nullptr || obj()->head.data == nullptr){
				LOG_DEBUG("SSZ", "Reference::listnewat addsize failed i=%d tsize=%d", (int)i, (int)tsize);
				return nullptr;
			}
		...
		if(null() || obj() == nullptr || obj()->head.data == nullptr){
			LOG_DEBUG("SSZ", "Reference::listnewat no storage i=%d tsize=%d", (int)i, (int)tsize);
			return nullptr;
		}
	...
	{
		if(s*tsize <= 0){
			// LOG_DEBUG("SSZ", "OutputFile::refnew ignored non-positive size s=%d tsize=%d", (int)s, (int)tsize);
			return;
		}
		HeapObj *tmp = (HeapObj*)sszrefnewfunc(sizeof(HeapObjHead)+s*tsize);
		if(tmp == nullptr){
			LOG_DEBUG("SSZ", "Reference::refnew alloc failed bytes=%d", (int)(sizeof(HeapObjHead)+s*tsize));
			return;
		}
	...
	{
		if(s*tsize <= 0){
			// LOG_DEBUG("SSZ", "Reference::listnew ignored non-positive size s=%d tsize=%d", (int)s, (int)tsize);
			return;
		}
		HeapObj *tmp = (HeapObj*)sszrefnewfunc(sizeof(HeapObj));
		if(tmp == nullptr){
			LOG_DEBUG("SSZ", "Reference::listnew alloc head failed");
			return;
		}
		tmp->head.data = (int8_t*)sszrefnewfunc(s*tsize);
		if(tmp->head.data == nullptr){
			LOG_DEBUG("SSZ", "Reference::listnew alloc data failed bytes=%d", (int)(s*tsize));
			sszrefdeletefunc(tmp);
			return;
		}
		...
		void* p = nullptr)
	{
		...
		if(null()){
			...
			if(null()){
				LOG_DEBUG("SSZ", "Reference::addsize listnew failed s=%d tsize=%d", (int)s, (int)tsize);
			}
			return 0;
		}
		...
		if(ds > obj()->body.limitsize){
			...
			if(tmpd == nullptr){
				LOG_DEBUG("SSZ", "Reference::addsize realloc failed newLimit=%d", (int)obj()->body.limitsize);
				obj()->body.limitsize = ds;
				return tmps;
			}
