...
{
	if(p == nullptr) return;
	if(pdata == nullptr || len <= 0) return;
	if(((RefNewStructure*)p)->newf == nullptr || ((RefNewStructure*)p)->csize <= 0) return;
...
struct BinaryCode
{
	...
	MEMBER void CallStk()
	{
		...
		CmpImm(0, false);
		NEqual();
		intptr_t nonNull = TrueJump(0);
		// null path: pop the saved func_ptr to balance the stack, then log and skip
		PopReg(EAX);
		intptr_t end = Jump(0);
		// non-null path:
		*(intptr_t*)(code.data()+nonNull) += code.size();
		PopReg(EAX);
		code += 0xFF;
		AtaiCode(ModRM(3, 2, EAX));
		*(intptr_t*)(code.data()+end) += code.size();
	#else
		code += 0xFF;
		AtaiCode(ModRM(3, 2, EAX));
	#endif
