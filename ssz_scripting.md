...
> Note: `len` is a *reserved* keyword (recognized by the tokenizer) but is **not
> implemented** by the compiler in this build. Do not use `len(...)` — use `#ref`
> for the length of a ref/list. (`len` appears only inside lazily-instantiated
> template bodies in the bundled scripts, which are never compiled.)
...
Launch a thread with the `..` (double-dot) call suffix; wait with `<wait>(t)`:

```ssz
.loadThread..();              // start the thread
...
- In this engine the plugin libraries are **statically linked and registered** at
  ...
  library for resolution — no DLL is loaded. The bundled libraries are: `ssz`,
  `lua`, `mesdialog`, `ogg`, `sdlplugin`, `alert`, `file`, `math`, `regex`, `shell`,
  `socket`, `sound`, `thread`, `time`.
...
### `typeid(expr)`, `typesize(expr)`, `sizeof(T)`
...
- `sizeof(T)` — byte size of a compile-time type (`sizeof(_t)*8`).
